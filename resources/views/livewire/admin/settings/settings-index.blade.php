<div>
    ...
    <form wire:submit="save" class="max-w-3xl space-y-6">
        ...
        {{-- Rate Sheet & Pricing PDF Configuration --}}
        <div class="rounded-xl border border-gray-200 bg-white">
            <div class="flex flex-col gap-3 border-b border-gray-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-sm font-semibold text-gray-900">{{ __('Rate Sheet & Pricing PDF') }}</h3>
                    <p class="text-xs text-gray-500">{{ __('Configure personal shopper tiers, heavy duty boxes, logistics fees, and bilingual notes for the official rate sheet.') }}</p>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    <a href="{{ route('admin.rates.pdf', ['lang' => 'es']) }}" target="_blank"
                       class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700 shadow-sm hover:bg-gray-50">
                    ...
                    <button type="button" wire:click="openSendModal"
                            class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-emerald-700">
            ...
            <div class="space-y-6 p-5">
                ...
                <div>
                    <h4 class="text-xs font-bold uppercase tracking-wider text-emerald-700">{{ __('1. Personal Shopper Tiers') }}</h4>
                    <div class="mt-3 space-y-3">
                        @foreach ($rates['shopper_tiers'] ?? [] as $index => $tier)
                            <div class="grid grid-cols-2 gap-3 rounded-xl border border-gray-200 bg-gray-50/60 p-3 sm:grid-cols-5 sm:items-center">
                                <div>
                                    <label class="block text-[11px] font-medium text-gray-500">{{ __('Min ($)') }}</label>
                                    <input type="number" step="1" wire:model="rates.shopper_tiers.{{ $index }}.min"
                                ...
                                <div>
                                    ...
                                    <input type="number" step="1" wire:model="rates.shopper_tiers.{{ $index }}.max"
                                           placeholder="+"
                                           class="mt-1 w-full rounded-lg border border-gray-300 bg-white px-2.5 py-1.5 text-xs focus:border-emerald-500 focus:ring-emerald-500">
                                </div>
                                <div>
                ...
                <div class="border-t border-gray-200 pt-5">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-emerald-700">{{ __('2. Repackaging & Warehouse Services') }}</h4>
                ...
                {{-- Notes & Policy Descriptions --}}
                <div class="border-t border-gray-200 pt-5">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-emerald-700">{{ __('3. Descriptive Notes & Policy (Bilingual)') }}</h4>
    ...
    @if ($showSendModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm"
             x-data x-on:keydown.escape.window="$wire.closeSendModal()">
            <div class="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
                <div class="flex items-center justify-between border-b border-gray-100 pb-4">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-gray-900">{{ __('Send Price List PDF') }}</h3>
                            <p class="text-xs text-gray-500">{{ __('The PDF will be attached and a copy will be sent to the administrator.') }}</p>
                        </div>
                    </div>
                    <button type="button" wire:click="closeSendModal" class="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600">
                ...
                <form wire:submit="sendRatesEmail" class="mt-4 space-y-4">
                    <div>
                        <label class="mb-1 block text-xs font-semibold text-gray-700">{{ __('Recipient Email') }} *</label>
                        <input type="email" wire:model="recipientEmail" placeholder="cliente@ejemplo.com" autofocus
                    ...
                    <div class="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50/70 p-3 text-xs text-emerald-800">
                        ...
                        <span>
                            ...
                            <strong>{{ ($settings['admin_notification_email'] ?? '') ?: __('Not configured (will only send to recipient)') }}</strong>
                        </span>
                    </div>
